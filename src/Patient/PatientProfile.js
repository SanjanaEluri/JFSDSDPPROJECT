...
export default function PatientProfile() {
  ...
  useEffect(() => {
    ...
    if (storedPatientData) {
      ...
      setImageSrc(`https://jfsdsdpbackend.up.railway.app/displaypatientimage?id=${parsedPatientData.id}`);
    }
  }, []);
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      const response = await axios.post("https://jfsdsdpbackend.up.railway.app/uploadimage", formDataToSend, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        setImageSrc(`https://jfsdsdpbackend.up.railway.app/displaypatientimage?id=${patientData.id}&timestamp=${Date.now()}`);
